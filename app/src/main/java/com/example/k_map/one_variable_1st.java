...
public class one_variable_1st extends AppCompatActivity {
    ...
    public void OkProceed(View view){
        ...
        if(TextUtils.isEmpty(editText.getText().toString()) || TextUtils.isEmpty(editText.getText().toString())
            ...
            Toast.makeText(one_variable_1st.this,"Empty fields not allowed",Toast.LENGTH_SHORT).show();
        }
        else {
            String n = editText.getText().toString();
            if(Integer.parseInt(n) >= 1 && Integer.parseInt(n) <=2) {
                Intent intent = new Intent(this, one_variable_2nd.class);
                intent.putExtra(key, n);
                startActivity(intent);
            }
            else{
                Toast.makeText(this, "Number you have entered is beyond 2 or less than 1", Toast.LENGTH_SHORT).show();
            }
        }
    }
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_one_variable1st);
    }
}